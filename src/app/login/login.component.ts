...
export class LoginComponent {
  ...
  {
    this.http.post("http://localhost:8080/login",{username:username,password:password},{observe:"response"}).subscribe(body=>{
      this.resp = body.body
  ...
  login1()
  {
    ...
    {
      this.http.post("http://localhost:8080/getquestions",{username:this.user.username,password:this.user.password},{observe:"response"}).subscribe(body=>{
