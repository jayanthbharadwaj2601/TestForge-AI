...
export class SignupComponent {
  ...
  {
    this.http.post("http://localhost:8080/getuser",{username:username,password:password},{observe:"response"}).subscribe(body=>{
      this.resp = body.body
  ...
  signup(username:string,password:string)
  {
    if(this.user.username==' ')
    {
      ...
      this.http.post("http://localhost:8080/signup",{username:this.user.username,password:this.user.password},{observe:"response"}).subscribe(body=>{
      this.resp = body.body
      console.log(this.resp)
    });
